window.masterData = [
    ...
    // --- CATEGORY: Altostrat Case Study ---
    {
        ...
        l: 'WAF pillars often have trade-offs; for example, increasing Reliability (HA) usually increases Cost.',
        c: 'The PCA exam frequently asks you to choose a solution that satisfies a specific pillar (e.g., "Which is the most cost-effective way...").',
    },
    {
        cat: 'Altostrat',
        q: 'Scenario: Altostrat needs to bridge on-premises media ingestion with cloud delivery. Requirements: Establish a reliable hybrid footprint for on-prem media ingestion, provide performant GKE environments in both locations, and allow on-prem pods to securely access Google Cloud APIs (Storage/Vertex) without static JSON service account keys.',
        a: 'GKE Enterprise (Anthos) + Dedicated Interconnect + Workload Identity Federation.',
        d: '1. **Connectivity**: 10/100 Gbps Dedicated Interconnect with Cloud Router (BGP) for SLA-backed throughput.\n2. **Orchestration**: GKE Enterprise (Anthos) provides a unified control plane for on-prem and GKE Autopilot cloud clusters.\n3. **Identity**: Workload Identity Federation allows on-prem GKE pods to assume IAM roles via a Security Token Service (STS).',
        ...
        c: 'For 99.99% HA, use 4 circuits (2 per metro across 2 metros) and 2 Cloud Routers. Use LACP (Link Aggregation) to bundle multiple 10G/100G circuits for increased throughput.',
    },
    {
        cat: 'Altostrat',
        q: 'Scenario: The library is growing rapidly and manual tagging is a bottleneck. Requirements: Auto-generate summaries and extract metadata from high volumes of video and images, ensure AI decisions are auditable/explainable, and minimize API overhead for multimodal analysis.',
        a: 'BigQuery ML (BQML) with Gemini 1.5 Pro Remote Models.',
        d: '1. **Trigger**: GCS Object Finalize triggers a Cloud Run function.\n2. **Intelligence**: BQML calls Gemini 1.5 Pro via a Remote Model connection. Gemini analyzes video directly for both tags and summaries in one call.\n3. **Audit**: Use Vertex AI Explainable AI for feature attribution and Cloud Logging for prompt/response audits.',
        ...
        c: 'Gemini 1.5 Pro multimodal capabilities replace the need for separate Vision, Video Intelligence, and Natural Language API calls.',
    },
    {
        cat: 'Altostrat',
        q: 'Scenario: Altostrat wants to increase engagement via 24/7 support. Requirements: Enable natural language user interactions grounded in company data, prevent prompt injection/harmful content, and ensure only valid subscribers can access the interface.',
        a: 'Vertex AI Search and Conversation + Model Armor + Identity-Aware Proxy (IAP).',
        d: '1. **Chatbot**: Vertex AI Agent grounded in the BigQuery metadata library for factual accuracy.\n2. **Security**: Model Armor acts as a firewall for LLM prompts to filter PII and injection attacks.\n3. **Access**: IAP validates user identity (IAM) at the HTTP layer before traffic reaches the GKE-hosted frontend.',
        ...
        c: 'IAP is a Zero Trust solution that eliminates the need for a traditional VPN for subscriber-only web access.',
    },
    {
        cat: 'Altostrat',
        q: 'Scenario: Storage volumes are scaling to Petabytes. Requirements: Optimize cloud storage costs for growing media volumes, move from on-premises CapEx to a cloud-based OpEx model, and ensure compute costs are only incurred during active processing.',
        a: 'Cloud Storage Autoclass + GKE Autopilot.',
        d: '1. **Storage**: Autoclass automatically transitions objects between Standard, Nearline, Coldline, and Archive based on access patterns.\n2. **Compute**: GKE Autopilot ensures you pay for Pod resource requests rather than idle, pre-provisioned Node capacity.\n3. **Lifecycle**: Use specific GCS Lifecycle Rules to delete temporary transcoding artifacts after 24 hours.',
